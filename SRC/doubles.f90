subroutine doubles(iprev,ilength,ispin,seed)
  use commonarrays, only: nword,nsym,nbft,nbpsy,irrep
  use mcci_in
  use precision
  use rng
  ...
  integer        :: nhalf, it
  integer        :: nhalf1, nhalf2
  real(kind=pr)  :: rand
  integer        :: iorb1, iorb2
  integer        :: icount, j, ja, n, jshift
  integer        :: j1shift, j1, n1
  integer        :: j2shift, j2, n2
  integer        :: isym1, isym2, ilo
  integer        :: k, ihi
  integer        :: l, i
  integer        :: irprod
  integer        :: j3shift, j3, n3
  integer        :: j4shift, j4, n4
  integer        :: isym3, isym4
  integer        :: irx
  integer        :: kold, kshift
  integer        :: lnew, lshift
  integer        :: iold1, iold2, ishift
  ...
  if(ispin.eq.1.or.ispin.eq.2) then
     ...
     it = 0
10   call rng_random(seed,rand)
     iorb1 = int(rand*(nhalf-nfreeze))+1
20   call rng_random(seed,rand)
...
40   call rng_random(seed,rand)
  ...
  elseif(ispin.eq.3) then
     ...
     it=0
50   call rng_random(seed,rand)
     iorb1 = int(rand*(nhalf1-nfreeze)) + 1

60   call rng_random(seed,rand)
