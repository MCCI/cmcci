subroutine reorder(ici,jci,i_am_mu,i_am_nu,nu_doubly,ndiff,idiff1,idiff2, &
                   kck,n_2p,ep)
  use commonarrays, only: icij, nword, nbft, ntotal, &
                          list, my_pair
  use mcci_in
  implicit none
  ...
  integer,       intent(out)    :: i_am_mu, i_am_nu
  integer,       intent(out)    :: nu_doubly, idiff1, idiff2, kck
  integer,       intent(inout)  :: ndiff
  integer,       intent(out)    :: n_2p, ep

  integer         :: idoubles(iword), jdoubles(iword)
  logical         :: spin1, spin2, swapped
  integer         :: n, m, k, l, kdoubly, ldoubly, ks_ld, ls_kd, l1, l2
  integer         :: j, jshift
  integer         :: ispin, jspin, kspin, lspin, itmp
        ...
        do m=1, ntotal
           if(n.ne.m) then
              if(list(i_am_mu,m).eq.list(i_am_nu,n))then
                 ...
                 !n = n - 1
                 !don't know what above comment means... MS
              endif
           endif
        enddo
     endif
